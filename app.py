...
def pagina():
    ...
    producer.send('ordenes', {"Nombre cocinero":request.form['Nombre cocinero'],"Nombre vendedor":request.form['Nombre vendedor'],"Cantidad sopaipillas vendidas":request.form["Cantidad sopaipillas vendidas"],"Correo vendedor":request.form["Correo vendedor"],"Correo cocinero":request.form["Correo cocinero"]})
    producer.flush()
...
def resumen():
    ...
    global contador
    print(contador)
    consumer = KafkaConsumer('ordenes',
    ...
    if bool(diccionario)==True:
        #se produce para el topico resumen
        producer = KafkaProducer(value_serializer=lambda v: json.dumps(v).encode('utf-8'), bootstrap_servers=['localhost:9092'])
        for cocinero in diccionario.keys():
            producer.send('resumen', {"Nombre cocinero":diccionario[cocinero],"Nombre vendedor":diccionario[cocinero]['Nombre vendedor'],"Cantidad sopaipillas vendidas":diccionario[cocinero]["Cantidad sopaipillas vendidas"],"Correo vendedor":diccionario[cocinero]["Correo vendedor"],"Correo cocinero":diccionario[cocinero]["Correo cocinero"]})
            producer.flush()
    ...
    for cocinero in diccionario.keys():
        ...
        Nombre vendedor: {diccionario[cocinero]["Nombre vendedor"]},\n
        Cantidad sopaipillas vendidas en total: {diccionario[cocinero]['Cantidad sopaipillas vendidas']}"""
